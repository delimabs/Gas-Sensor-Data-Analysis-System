...
class GasSensorDataAnalysisSystem(QMainWindow):
    def __init__(self):
        ...
        self.setWindowTitle('Gas Sensor Data Analysis System v0.9.3')
    ...
    def openResponseDialog(self):
        ...
        self.clearRespBtn.setText('Clear last')
    ...
    def openSettingsDialog(self):
        ...
        self.concentrationUnitInput.setPlaceholderText(
            self.concentrationUnitStr)
        ...
        self.settingsDlgWidget1Layout.addWidget(
            self.sensitivityOpt, 4, 0, 1, 1)

        self.settingsDlgWidget1Layout.addWidget(
            self.concentrationUnitLbl, 1, 1, 1, 1)
        self.settingsDlgWidget1Layout.addWidget(
            self.concentrationUnitInput, 1, 2, 1, 1)

        self.settingsDlgWidget1Layout.addWidget(
            self.numberOfFitPointsLbl, 2, 1, 1, 1)
        self.settingsDlgWidget1Layout.addWidget(
            self.numberOfFitPointsInput, 2, 2, 1, 1)
        ...
        self.closeSettingsBtn.clicked.connect(self.settingsDlg.reject)
    ...
    def fitRespData(self):
        ...
        for i, column in enumerate(self.visualizationDF.columns):
            ...
            popt = curve_fit(self.powerLawFunc, self.propertiesDF['concentration'],
    ...
    def exportData(self):
            ...
            if self.responseType['dR/R0']:
                if self.responseType['sensitivity']:
                    self.propDataFile.write(
                        f'# Sensitivity in %/{self.concentrationUnitStr}\n')
                else:
                ...
                if self.responseType['sensitivity']:
                    self.propDataFile.write(
                        f'# Sensitivity in {self.channelsUnitStr}/{self.concentrationUnitStr}\n')
                else:
                    self.propDataFile.write(
                ...
                if self.responseType['sensitivity']:
                    self.propDataFile.write(
                        f'# Sensitivity in Rgas/Rair/{self.concentrationUnitStr}\n')
                else:
                    self.propDataFile.write(f'# Response in Rgas/Rair\n')
        ...
        if self.exportFitInfoCheck.isChecked():
            ...
            for label in self.fitListLabel:
